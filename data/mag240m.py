...
from .dataloader import NeighborTask, MultiTaskSplitWay, MultiTaskSplitBatch, MulticlassTask, ParamSampler, BatchSampler, Collator, ContrastiveTask, CentroidSamplingTask
...
def get_mag240m_dataloader(dataset, task_name, split, node_split, batch_size, n_way, n_shot, n_query, batch_count, root, num_workers, aug, aug_test, use_subset=True, subset_size=20000000, centroids_path=None, sampling_mode='alternate', **kwargs):
    ...
    if task_name == "same_graph":
        ...
        # Use CentroidSamplingTask with the specified mode if centroids_path is provided
        centroids = None
        if centroids_path and os.path.exists(centroids_path) and split == "train":
            print(f"Loading centroids from {centroids_path} for {sampling_mode} node selection")
            centroids = torch.load(centroids_path)
            
            # Print the available sampling modes for clarity
            valid_modes = ['random', 'alternate', 'mixed', 'centroids_only', 'mixed_batch']
            print(f"Using sampling mode: {sampling_mode} (valid options: {', '.join(valid_modes)})")
            
            # Warn about using mixed_batch mode with limited centroids
            if sampling_mode == 'mixed_batch':
                num_batches_possible = len(centroids) // (n_way // 2)
                print(f"With {len(centroids)} centroids and {n_way} way (using {n_way//2} centroids per batch), "
                      f"we can create approximately {num_batches_possible} unique batches")
                if num_batches_possible < batch_count:
                    print(f"Warning: Requested {batch_count} batches but can only create approximately "
                          f"{num_batches_possible} unique batches with current centroids")
        
        # Create task with the appropriate sampling mode
        task = CentroidSamplingTask(
            neighbor_sampler, 
            len(dataset), 
            "inout", 
            centroids=centroids, 
