...
from centroid_neighbour_taskV2 import CentroidOnlyNeighborTask
...
def patched_get_mag240m_dataloader(dataset, task_name, split, node_split, batch_size, n_way, n_shot, n_query, 
                                  ...
                                  **kwargs):
    """Patched version that supports centroid-only sampling"""
    ...
    if task_name == "neighbor_matching" and centroids is not None:
        print(f"Using centroid-only neighbor task sampling with {n_way} centroids per batch")
        ...
        # Use custom CentroidOnlyNeighborTask - no mixing with random nodes
        task = CentroidOnlyNeighborTask(neighbor_sampler, len(dataset), "inout", centroids)
