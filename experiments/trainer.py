...
from data.two_part_augmentation import get_two_part_aug
...
class TrainerFS():
    def __init__(self, dataset, parameter):
        ...
        # Check if we should apply augmentation auto-conversion
        # If parameter starts with TP: or G, it's already using our new format
        # Otherwise, we'll auto-convert standard augmentation to two-part global
        self.use_two_part_aug = False
        aug_spec = self.parameter["augmentation"]
        if aug_spec and (aug_spec.startswith("TP:") or aug_spec.startswith("G")):
            self.use_two_part_aug = True
            print(f"Using two-part global augmentation: {aug_spec}")
        ...
        self.connect_same_label_nodes = self.parameter.get("connect_same_label_nodes", False)
        ...
        if self.parameter["layers"] != "SimpleDotProduct":
            ...
            layer_list = get_module_list(self.parameter["layers"], self.emb_dim, edge_attr_dim=edge_attr_dim,
                                        ...
                                        connect_same_label_nodes=self.connect_same_label_nodes)
    ...
    # 新增for try Stiefel retraction
    @torch.no_grad()
    def reorthogonalize(self):
        """QR‑based retraction onto Stiefel manifold."""
        if not hasattr(self.model, 'learned_label_embedding'):
            return
        
        # 获取当前批次实际使用的标签数
        current_n_way = getattr(self, 'current_batch_n_way', self.ways)
        
        W = self.model.learned_label_embedding.weight  # (1000, 256)
        emb_dim = W.shape[1]  # 256
        
        # 确保维度条件满足
        if current_n_way <= emb_dim:
            # 只对前current_n_way行进行正交化
            W_subset = W[:current_n_way, :]  # (30, 256)
            Q, _ = torch.linalg.qr(W_subset.T, mode='reduced')  # Q: (256, 30)
            W[:current_n_way, :].copy_(Q.T)  # 更新: (30, 256)
        else:
            print(f"Skipping orthogonalization: n_way={current_n_way} > emb_dim={emb_dim}")                                      # 回转并就地覆盖，保持梯度图                              # (C, d)，正交且单位范数

    def get_orthogonal_loss(self, final_label_emb=None):
        """
        Compute the orthogonality loss of the final label embeddings using cross entropy.
        
        CE(O * O^T, I) where:
        - O is the matrix of normalized label embeddings after task graph propagation
        - I is the identity matrix
        - CE is cross entropy loss
        
        :param final_label_emb: Label embeddings after task graph propagation
        :return: Scalar loss.
        """
        if final_label_emb is None:
            return torch.tensor(0.0, device=self.device)
        
        # Normalize the label embeddings
        label_embeddings_norm = torch.nn.functional.normalize(final_label_emb, p=2, dim=1)
        
        # Compute the dot product matrix
        dot_product = torch.matmul(label_embeddings_norm, label_embeddings_norm.transpose(0, 1))
        
        # Create a target tensor for cross entropy (indices of the identity matrix)
        target = torch.arange(dot_product.shape[0], device=dot_product.device)
        
        # Compute the loss using cross entropy
        loss = torch.nn.functional.cross_entropy(dot_product, target)
    ...
    def _build_dataloaders(self, dataset, dataset_name):
        ...
        # Add support for two-part augmentation
        kwargs["use_two_part_aug"] = self.use_two_part_aug
        
        # Add centroids parameters
        if "centroids_path" in self.parameter:
            kwargs["centroids_path"] = self.parameter["centroids_path"]
            print(f"Passing centroids_path: {self.parameter['centroids_path']}")
        
        if "sampling_mode" in self.parameter:
            kwargs["sampling_mode"] = self.parameter["sampling_mode"]
            print(f"Passing sampling_mode: {self.parameter['sampling_mode']}")
        elif "sampling-mode" in self.parameter:
            kwargs["sampling_mode"] = self.parameter["sampling-mode"]
            print(f"Passing sampling_mode: {self.parameter['sampling-mode']}")
        
        # Add MAG240M subset parameters 
        if dataset_name == "mag240m":
            kwargs["use_subset"] = self.parameter.get("use_mag_subset", True)
            kwargs["subset_size"] = self.parameter.get("mag_subset_size", 20000000)
    ...
    def train(self):
        ...
        with torch.no_grad():
            test_loss, test_acc, test_acc_std, test_aux_loss, test_orthogonal_loss, ranks = self.do_eval(self.test_dataloader)
            start_log_dict = {"start_test_acc": test_acc, "start_test_acc_std": test_acc_std}
            if ranks is not None:
                for key in ranks:
                    start_log_dict["start_test_" + key] = ranks[key]
            wandb.log(start_log_dict)

        # Also update in the eval_only section
        if "eval_only" in self.parameter and self.parameter["eval_only"]:
            ...
            with torch.no_grad():
                val_loss, val_acc, val_acc_std, val_aux_loss, val_orthogonal_loss, ranks = self.do_eval(self.val_dataloader)
                ...
                print(f"Test accuracy std: {test_acc_std}")
        ...
        for e in pbar:
            ...
            # add this to get the real number of labels in the batch
            if len(batch) >= 2:  # 确保batch有足够的元素
                x_label = batch[1]  # 通常x_label是第二个元素
                self.current_batch_n_way = x_label.shape[0]
            else:
                self.current_batch_n_way = self.ways  # 回退到默认值
            
            yt, yp, graph, final_label_emb = self.model(*batch)  # apply the model
            loss, acc = self.get_loss_and_acc(yt, yp)  # get loss
            aux_loss = self.get_aux_loss(graph)
            orthogonal_loss = self.get_orthogonal_loss(final_label_emb) if self.parameter["orthogonal_loss_weight"] > 0 else torch.tensor(0.0, device=self.device)
            weight = self.parameter["attr_regression_weight"]
            orthogonal_weight = self.parameter["orthogonal_loss_weight"]
            total_loss = loss + aux_loss * weight #+ orthogonal_loss * orthogonal_weight # not use for now to try Stiefel retraction
            total_loss.backward()
            self.optimizer.step()
            self.reorthogonalize()   # <- 新增for try Stiefel retraction

            # Update the wandb logging:
            wandb.log({
                "train_loss": loss, 
                "train_acc": acc, 
                "train_aux_loss": aux_loss, 
                #"train_orthogonal_loss": orthogonal_loss.item() if orthogonal_weight > 0 else 0,
            ...
            if e % self.eval_step == 0 and e != 0:
                ...
                # In the train method, update the validation evaluation
                with torch.no_grad():
                    self.model.eval()
                    val_loss, val_acc, val_acc_std, val_aux_loss, val_orthogonal_loss, ranks = self.do_eval(self.val_dataloader)
                ...
                pbar.write(f"Validation loss {val_loss} acc {val_acc} aux_loss {val_aux_loss} orthogonal_loss {val_orthogonal_loss}")
                wandb.log({
                    "valid_loss": val_loss, 
                    "valid_acc": val_acc, 
                    "valid_aux_loss": val_aux_loss,
                    "valid_orthogonal_loss": val_orthogonal_loss
                }, step=e)
                ...
                with torch.no_grad():
                    ...
                    log_dict = {
                        ...
                        "test_orthogonal_loss": test_orthogonal_loss
                    }
                    if ranks is not None:
    ...
    def do_eval(self, dataloader, eff_len=None):
        ...
        for batch in tqdm(dataloader, leave=False):
            ...
            acc_all.append(acc)
            aux_loss = self.get_aux_loss(graph)
            # We'll accumulate orthogonal losses separately if needed
            if self.parameter["orthogonal_loss_weight"] > 0:
                orthogonal_loss = self.get_orthogonal_loss(final_label_emb)
                all_orthogonal_loss.append(orthogonal_loss.item())
            if self.calc_ranks:
                task_mask = batch[9]
        ...
        orthogonal_loss_global = sum(all_orthogonal_loss) / len(all_orthogonal_loss) if all_orthogonal_loss else 0.0
        torch.set_grad_enabled(True)
        if ranks is not None:
            ranks = {key: np.average([r[0][key] for r in ranks], weights=[r[1] for r in ranks]) for key in ranks[0][0]}
        return loss_global, acc_global, acc_batch_std, aux_loss_global, orthogonal_loss_global, ranks