...
def get_params():
    ...
    # MAG240M subset parameters
    args.add_argument("-use_mag_subset", "--use_mag_subset", default=True, type=bool)  # Whether to use the MAG240M subset
    args.add_argument("-mag_subset_size", "--mag_subset_size", default=20000000, type=int)  # Size of the MAG240M subset
    ...
    # Add centroids for controlled sampling
    args.add_argument("--centroids_path", type=str, default=None, 
                    help="Path to centroids indices file for controlled sampling")
    args.add_argument("--num_centroids", type=int, default=10, 
                    help="Number of centroids to use per batch")
    args.add_argument("--sampling-mode", "--sampling_mode", type=str, default='alternate',
                 choices=['alternate', 'mixed', 'random', 'centroids_only', 'mixed_batch'],
                 help="Mode for centroid sampling: 'alternate' (50/50 batches), 'mixed' (centroids+random), 'mixed_batch' (50/50 within each batch), 'centroids_only', or 'random'")
    ...
    args.add_argument("-connect_same_label", "--connect_same_label_nodes", default=False, type=bool)  # Connect nodes with the same label
    args.add_argument("-smalldataset", "--small_dataset", default=False,
                      type=bool)  # use for debugging  - very small dataset
    args.add_argument("-orth", "--orthogonal_loss_weight", default=0.0, type=float, help="Weight for orthogonal loss to make label embeddings orthogonal")
    args.add_argument("-exptype", "--experiment_type", default="metagraph")
    args.add_argument("-mean_label_emb", "--mean_label_embeddings", default=False, type=bool,
                  help="Use mean of task graph input nodes to initialize label embeddings")
