...
class SingleLayerGeneralGNN(torch.nn.Module):
    ...
    def forward(self, graph, x_label, y_true_matrix, metagraph_edge_index, metagraph_edge_attr, query_set_mask, input_seqs=None, query_seqs=None, query_seqs_gt=None, task_mask=None):
        ...
        :return: y_true_matrix, y_pred_matrix (for the query set only!), graph, final_label_emb
        '''
        ...
        if self.params["ignore_label_embeddings"]:
            if self.params.get("mean_label_embeddings", False):
                # Use the mean of input node features for each class as label embeddings
                # Get supernode embeddings for the label initialization
                supernode_feats = graph.x[supernode_idx]
                
                # HARDCODED APPROACH: Based on the actual feature dimension, decide what to do
                # This is the most reliable approach to avoid any issues with calculations
                feat_dim = supernode_feats.shape[1]
                
                # Just print for debug - can be removed later
                
                # Features are already 256-dimensional (emb_dim) - use directly
                if feat_dim == 256:
                    supernode_embs = supernode_feats
                # Features are 768-dimensional (input_dim) - project normally
                elif feat_dim == 768:
                    supernode_embs = self.initial_label_mlp(supernode_feats)
                # Features are 770-dimensional (input_dim + 2 flags) - trim and project
                elif feat_dim == 770:
                    supernode_embs = self.initial_label_mlp(supernode_feats[:, :768])
                # Any other dimension - raise error with helpful message
                else:
                    raise ValueError(f"Unexpected feature dimension: {feat_dim}. Expected 256, 768, or 770.")
                
                # Reshape the query mask to match the query indices
                qry_mask_reshaped = query_set_mask.reshape(-1, y_true_matrix.shape[1])[:, 0]
                
                # Create support mask (opposite of query mask)
                support_mask = ~qry_mask_reshaped
                
                # Initialize new label embeddings
                new_x_label = torch.zeros_like(x_label)
                
                # For each class
                for c in range(y_true_matrix.shape[1]):
                    # For support samples, use task_mask if available
                    if task_mask is not None:
                        # Use task_mask to identify support samples for this class
                        class_mask = (task_mask == c) & support_mask
                    else:
                        # Without task_mask, try to reconstruct class info from one-hot encoding
                        # This is tricky - we'll use the most general approach possible
                        # Assume any non-query sample with a 1 in this column is a support sample for this class
                        all_samples = torch.zeros_like(qry_mask_reshaped, dtype=torch.bool)
                        all_samples[support_mask] = True
                        class_mask = all_samples
                    
                    if class_mask.sum() > 0:
                        # Get embeddings of support samples for this class
                        class_features = supernode_embs[class_mask]
                        # Set label embedding to mean of class features
                        new_x_label[c] = class_features.mean(dim=0)
                    else:
                        # If no support samples for this class, keep original embedding
                        new_x_label[c] = x_label[c]
        ...
        return y_true_matrix[qry_idx, :], y_pred_matrix[qry_idx, :], graph, x_label