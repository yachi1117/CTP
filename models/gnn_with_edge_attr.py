...
class SAGEConvSelfLoops(MessagePassing):
    ...
    def __init__(self, x_dim, edge_attr_dim, emb_dim, dropout = 0, aggr="add", transform_x=True, batch_norm=True):
        super(SAGEConvSelfLoops, self).__init__(aggr=aggr)
        # multi-layer perceptron
    ...
    def forward(self, x, edge_index, edge_attr=None):
        ...
        # Process edge attributes outside propagate
        self.edge_attr_emb = None
        if self.lin_edge_attr is not None and edge_attr is not None:
            self.edge_attr_emb = self.lin_edge_attr(edge_attr)
        
        # Call propagate without edge_attr parameter
    ...
    def message(self, x_j, edge_index, size):
        # Access edge attributes from instance variable
        if self.edge_attr_emb is None:
            return x_j
        else:
            # Get edge index for indexing into edge_attr_emb
            edge_idx = edge_index[1]  # Using target index
            return x_j + self.edge_attr_emb
