...
class MetaGNN(torch.nn.Module, MetagraphLayer):
    def __init__(self, edge_attr_dim, emb_dim, heads = 8, n_layers=1, dropout = 0, aggr="add", has_final_back=False, msg_pos_only = False, self_loops = True, batch_norm = True, gat_layer = False, use_relu=False, connect_same_label_nodes=False):
        super().__init__()
        self.num_gnn_layers = n_layers
        self.gnn_layers = torch.nn.ModuleList()
        self.msg_pos_only = msg_pos_only
        self.self_loops = self_loops
        self.connect_same_label_nodes = connect_same_label_nodes
    ...
    def forward(self, x, edge_index, edge_attr, query_mask, start_right, **kwargs):
        ...
        # If connect_same_label_nodes is True, add edges between nodes that share the same label
        original_edge_count = edge_index.shape[1]
        if self.connect_same_label_nodes:
            # Identify nodes that belong to the same label
            node_to_label_mask = (edge_index[0] < start_right) & (edge_index[1] >= start_right)
            node_to_label_edges = edge_index[:, node_to_label_mask]
            node_to_label_attr = edge_attr[node_to_label_mask]
            
            # Only consider support edges (not query edges) where the node belongs to the label
            positive_label_mask = (~node_to_label_attr[:, 0].bool()) & (node_to_label_attr[:, 1] > 0)
            node_to_label_edges = node_to_label_edges[:, positive_label_mask]
            
            # Group nodes by the label they connect to
            label_to_nodes = {}
            for i in range(node_to_label_edges.size(1)):
                node = node_to_label_edges[0, i].item()
                label = node_to_label_edges[1, i].item()
                if label not in label_to_nodes:
                    label_to_nodes[label] = []
                label_to_nodes[label].append(node)
            
            # Add edges between all pairs of nodes that share the same label
            new_edges = []
            for label, nodes in label_to_nodes.items():
                for i in range(len(nodes)):
                    for j in range(i + 1, len(nodes)):
                        new_edges.append([nodes[i], nodes[j]])
                        new_edges.append([nodes[j], nodes[i]])  # Add the reverse edge as well
            ...
            if new_edges:
                ...
                # Assuming these are support edges (since they connect support nodes)
                # Set them to [False, 1.0] for [is_query, label_assignment]
                new_edge_attr = torch.zeros(new_edges.size(1), edge_attr.size(1), device=edge_attr.device)
                new_edge_attr[:, 1] = 1.0  # Positive label assignment
                edge_attr = torch.cat([edge_attr, new_edge_attr], dim=0)
                
                # Extend the query_mask tensor to account for the new edges
                new_query_mask = torch.zeros(new_edges.size(1), dtype=torch.bool, device=query_mask.device)
                query_mask = torch.cat([query_mask, new_query_mask], dim=0)
                
                # Re-calculate the support mask with the new extended query mask
                if not self.msg_pos_only:
                    support_mask = (~query_mask)
                else:
                    positives = edge_attr[:, -1] == 1  # remove negatives
        ...
        query_in_mask = query_mask[:edge_index.shape[1]]  # Ensure query_in_mask matches edge_index size
